...
def render_header() -> None:
    st.markdown('<div class="main-title">TikTok Fashion Prompt Studio</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">美区女装原生视频提示词工作台 · 快速款式识别 · 场景规划 · 文本提示词输出</div>', unsafe_allow_html=True)
    st.markdown(
        """
        <div class="hero-card">
        ...
        if st.button("生成款式资产卡", type="primary", use_container_width=True):
            try:
                with st.spinner("正在提取服装可见结构..."):
                    api_image_bytes, mime_type, compress_note = compress_image_for_api(image_bytes, max_mb=0.8)
                ...
                with st.spinner("正在整理款式资产卡..."):
                    ...
                    asset_card = call_text_model(asset_prompt, temperature=0.2, max_tokens=1800)
                ...
                st.markdown(
                    "<div class='warning-soft'>建议检查：API Key、模型 ID、图片理解能力、图片清晰度。正式使用建议上传 1–3MB 的清晰参考图。</div>",
                    unsafe_allow_html=True,
                )
...
def render_generation(settings: Dict[str, Any]) -> None:
    st.subheader("4. 生成结果")
    st.markdown("<div class='section-note'>本工具只输出 Seedance 2.0 文本提示词，不生成图片或视频。</div>", unsafe_allow_html=True)
    if st.button("生成 Seedance 2.0 文本提示词", type="primary", use_container_width=True, disabled=not bool(st.session_state.clothing_profile.strip())):
        ...
        with st.spinner("第 2 步：正在生成 Seedance 2.0 文本提示词..."):
            try:
