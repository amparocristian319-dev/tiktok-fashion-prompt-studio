...
def render_api_center() -> None:
    ...
    with st.expander("模型接入中心 · Seed 2.1 Pro", expanded=not is_configured):
        ...
        if mode == "页面配置":
            ...
            st.markdown("#### 图片 URL 上传")
            st.markdown("<div class='section-note'>为避免 base64 大请求体写入超时，推荐把拖入的图片先自动上传到七牛云，再把图片 URL 发给图片识别模型。</div>", unsafe_allow_html=True)
            qiniu_col1, qiniu_col2 = st.columns(2)
            with qiniu_col1:
                qiniu_access_key = st.text_input(
                    "Qiniu AccessKey",
                    value=current.get("QINIU_ACCESS_KEY", ""),
                    type="password",
                    placeholder="你的七牛 AccessKey",
                )
                qiniu_secret_key = st.text_input(
                    "Qiniu SecretKey",
                    value=current.get("QINIU_SECRET_KEY", ""),
                    type="password",
                    placeholder="你的七牛 SecretKey",
                )
                qiniu_bucket = st.text_input(
                    "Qiniu Bucket",
                    value=current.get("QINIU_BUCKET", "tiktok-fashion-vision-suryus"),
                    placeholder="tiktok-fashion-vision-suryus",
                )
            with qiniu_col2:
                qiniu_domain = st.text_input(
                    "Qiniu 图片访问域名",
                    value=current.get("QINIU_DOMAIN", "thhne79gp.hd-bkt.clouddn.com"),
                    placeholder="thhne79gp.hd-bkt.clouddn.com",
                    help="测试域名一般不支持 HTTPS，所以先不要带 http:// 或 https://。",
                )
                qiniu_scheme = st.selectbox(
                    "图片 URL 协议",
                    ["http", "https"],
                    index=0 if current.get("QINIU_SCHEME", "http") == "http" else 1,
                    help="你当前七牛测试域名不支持 HTTPS，先用 http。后续绑定自定义域名和证书后再改 https。",
                )
                qiniu_prefix = st.text_input(
                    "图片保存目录",
                    value=current.get("QINIU_PREFIX", "fashion-uploads"),
                    placeholder="fashion-uploads",
                )
            ...
            with c1:
                if st.button("保存配置", use_container_width=True):
                    runtime_config = {
                        ...
                        "QINIU_ACCESS_KEY": qiniu_access_key.strip(),
                        "QINIU_SECRET_KEY": qiniu_secret_key.strip(),
                        "QINIU_BUCKET": qiniu_bucket.strip(),
                        "QINIU_DOMAIN": qiniu_domain.strip(),
                        "QINIU_SCHEME": qiniu_scheme,
                        "QINIU_PREFIX": qiniu_prefix.strip() or "fashion-uploads",
                    }
...
# 七牛云 Kodo 图片 URL 上传，解决 base64 write timeout
QINIU_ACCESS_KEY = "你的七牛 AccessKey"
QINIU_SECRET_KEY = "你的七牛 SecretKey"
QINIU_BUCKET = "{qiniu_bucket.strip() or 'tiktok-fashion-vision-suryus'}"
QINIU_DOMAIN = "{qiniu_domain.strip() or 'thhne79gp.hd-bkt.clouddn.com'}"
QINIU_SCHEME = "{qiniu_scheme}"
QINIU_PREFIX = "{qiniu_prefix.strip() or 'fashion-uploads'}"
