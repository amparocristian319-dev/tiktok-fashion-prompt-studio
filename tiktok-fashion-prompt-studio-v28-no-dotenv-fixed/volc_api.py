...
def call_vision_model(
    ...
    if not image_url and not image_bytes:
        raise RuntimeError("图片识别缺少图片输入：image_url 或 image_bytes 至少需要一个。")

    def build_payload(url_value: str) -> Dict[str, Any]:
        payload: Dict[str, Any] = {
    ...
    # Preferred path: send the public image URL only. This avoids large base64 request-body write timeout.
    if image_url:
        _record_api_event({
            "stage": "vision_url_ready",
            "type": "图片识别",
            "mode": "public_url",
        ...
        print(f"[vision-url] model={get_vision_model_id()} url={image_url[:160]} thinking={thinking_type}")
        return _post_chat(
            build_payload(image_url),
            "图片识别(public_url)",
            timeout_seconds=request_timeout or get_asset_timeout_seconds(),
            retry=0,
        )

    # Fallback path: base64. Keep it small, because Streamlit Cloud can timeout while writing large JSON bodies.
    ...
    for idx, mode in enumerate([primary_mode, fallback_mode]):
        ...
        _record_api_event({
            ...
            "base64_chars": len(image_payload),
            "model": get_vision_model_id(),
            "thinking": thinking_type,
        })
        ...
        try:
            ...
            if "超时" in msg or "write operation timed out" in msg.lower():
                break
