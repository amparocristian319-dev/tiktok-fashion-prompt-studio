...
def render_api_center() -> None:
    ...
    with st.expander("API 配置中心 · Doubao Seed 2.1 Pro", expanded=not is_configured):
        ...
        if mode == "临时页面配置（适合测试）":
            ...
            secrets_text = f"""ARK_API_KEY = "{api_key_input.strip() or '你的火山方舟 API Key'}"
ARK_BASE_URL = "{base_url_input.strip() or 'https://ark.cn-beijing.volces.com/api/v3'}"
...
ARK_VISION_MODEL_ID = "{vision_model_input.strip() or '图片理解模型ID或Endpoint ID'}" """
            st.markdown("<div class='section-note'>复制下面内容到 Streamlit Secrets，可长期保存。</div>", unsafe_allow_html=True)
