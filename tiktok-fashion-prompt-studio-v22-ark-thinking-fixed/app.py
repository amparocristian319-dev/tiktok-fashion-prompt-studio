...
def render_api_center() -> None:
    ...
    request_timeout = status.get("request_timeout", "120")
    ...
    with st.expander("模型接入中心 · Seed 2.1 Pro", expanded=not is_configured):
        ...
        st.markdown(
            ...
            <div class="api-grid">
              ...
              <div class="api-card"><div class="api-k">Timeout</div><div class="api-v">{request_timeout}s</div></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if mode == "页面配置":
            ...
            with col2:
                ...
                image_input_mode = st.selectbox(
                    ...
                    help="默认 raw_base64；若格式不兼容，系统会尝试备用格式。",
                )
                request_timeout = st.selectbox(
                    "请求等待时间",
                    [60, 90, 120, 180, 240, 300],
                    index=[60, 90, 120, 180, 240, 300].index(int(current.get("ARK_REQUEST_TIMEOUT", "180"))) if str(current.get("ARK_REQUEST_TIMEOUT", "180")).isdigit() and int(current.get("ARK_REQUEST_TIMEOUT", "180")) in [60, 90, 120, 180, 240, 300] else 2,
                    help="图片识别可能比文本慢。正式使用建议 120 秒；调试接口问题时可选 60 秒。",
                )
