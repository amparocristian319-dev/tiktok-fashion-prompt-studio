...
def render_api_center() -> None:
    ...
    with st.expander("API Control Center · 豆包 Seed 2.1 Pro 接入状态", expanded=False):
        st.markdown(
            f"""
            <div class="api-grid">
              <div class="api-card"><div class="api-k">API Key</div><div class="api-v">{api_key_status}</div></div>
        ...
        st.markdown(
            "<div class='section-note'>密钥不要写进 GitHub。正式部署时只在 Streamlit Cloud → App → Settings → Secrets 里配置。</div>",
            unsafe_allow_html=True,
        )
        st.code(
...
""",
        ...
        st.markdown(
            "<div class='section-note'>只填 DOUBAO_SEED21_PRO_MODEL_ID 时，系统会同时用于图片资产分析、场景深度规划、Seedance 文本提示词生成。</div>",
            unsafe_allow_html=True,
        )
