...
def make_tiny_test_image() -> tuple[bytes, str]:
    """Create a tiny local JPEG for vision endpoint testing."""
    img = Image.new("RGB", (320, 320), color=(245, 245, 245))
    # Keep it simple: no text drawing dependency. The model only needs a valid image payload.
    buf = io.BytesIO()
    img.save(buf, format="JPEG", quality=85, optimize=True)
    return buf.getvalue(), "image/jpeg"


def run_vision_debug_call(test_prompt: str, image_bytes: bytes, mime_type: str) -> str:
    """Call the current vision model with a tiny prompt and return raw response or raised error."""
...
def render_header() -> None:
    st.markdown('<div class="main-title">TikTok Fashion Prompt Studio</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">美区女装原生视频提示词生成器 · Doubao Seed 2.1 Pro 接入 · 场景深度规划 · 只生成文本</div>', unsafe_allow_html=True)
    st.markdown(
        """
        <div class="hero-card">
          <div class="metric-grid">
            <div class="metric-card"><div class="metric-label">模型接入</div><div class="metric-value">火山方舟 / 豆包 Seed 2.1 Pro</div></div>
...
def render_api_center() -> None:
    ...
    with st.expander("API 配置中心 · Doubao Seed 2.1 Pro", expanded=not is_configured):
        st.markdown(
            """
            <div class="api-shell">
              <div class="api-side">
                <div class="api-side-title">设置</div>
                <div class="api-side-item api-side-item-active">🔑 API 配置</div>
                <div class="api-side-item">⚙️ 模型路由</div>
                <div class="api-side-item">🧪 连接测试</div>
                <div class="api-side-item">🛡️ 安全说明</div>
              </div>
              <div>
                <div class="api-main-title">Doubao Seed 2.1 Pro 接入</div>
        ...
        with test_col1:
            if st.button("测试文本连接", use_container_width=True):
                try:
                    result = call_text_model("请只回复：text ok", temperature=0.1, max_tokens=20)
                    st.success(f"文本连接成功：{result}")
                except Exception as exc:
                    st.error(f"文本连接失败：{exc}")
        with test_col2:
            if st.button("测试图片理解", use_container_width=True):
                try:
                    tiny_img, tiny_mime = make_tiny_test_image()
                    st.info("Step 1/4：已生成 320×320 测试图片。")
                    st.info("Step 2/4：准备调用 Vision Model。")
                    result = run_vision_debug_call("请只用一句中文描述这张图片的主要内容。", tiny_img, tiny_mime)
                    st.success("图片理解连接成功。")
                    st.write(result)
                except Exception as exc:
                    st.error(f"图片理解测试失败：{exc}")
                    st.markdown(
                        "<div class='warning-soft'>如果文本连接成功但图片理解失败，说明当前 Vision Endpoint 或图片传输格式不兼容。请切换 data_url / raw_base64，或换支持图片理解的 ARK_VISION_MODEL_ID。</div>",
                        unsafe_allow_html=True,
                    )

        mode = st.radio(
            "配置方式",
            ["临时页面配置（适合测试）", "Streamlit Secrets 长期配置（推荐上线）"],
            horizontal=True,
            index=0,
            help="临时页面配置只在当前会话中生效；刷新或重启后可能丢失。上线长期使用请填 Streamlit Secrets。",
        )
        ...
        if mode == "临时页面配置（适合测试）":
            ...
            with col1:
                st.selectbox(
                    "服务商类型",
                    ["火山方舟 / 豆包 Seed 2.1 Pro", "OpenAI 兼容接口"],
                    index=0,
                )
            ...
            with col2:
                ...
                image_input_mode = st.selectbox(
                    ...
                    help="如果图片理解一直卡住或报 unsupported image_url，可以切换 raw_base64 再试。",
                )
            ...
            with c1:
                if st.button("保存到当前会话", use_container_width=True):
                    runtime_config = {
                    ...
                    st.success("已保存到当前会话。现在可以直接测试图片分析和提示词生成。")
                    st.rerun()
            with c2:
                if st.button("清空临时配置", use_container_width=True):
                    st.session_state["runtime_api_config"] = {}
                    st.warning("已清空当前会话 API 配置。")
                    st.rerun()

            secrets_text = f"""ARK_API_KEY = "你的火山方舟 API Key"
ARK_BASE_URL = "{base_url_input.strip() or 'https://ark.cn-beijing.volces.com/api/v3'}"
...
ARK_VISION_MODEL_ID = "{vision_model_input.strip() or '图片理解模型ID或Endpoint ID'}" """
            st.markdown("<div class='section-note'>复制下面内容到 Streamlit Secrets，可长期保存。为安全起见，这里不会回显真实 API Key。</div>", unsafe_allow_html=True)
            st.code(secrets_text, language="toml")

        else:
            st.markdown(
                "<div class='section-note'>长期上线请在 Streamlit Cloud 后台填写，不要把密钥写进 GitHub 文件。</div>",
                unsafe_allow_html=True,
            )
            st.code(
...
# ARK_VISION_MODEL_ID = "图片理解模型ID或Endpoint ID" """,
            ...
            st.markdown(
                "<div class='section-note'>路径：Streamlit Cloud → 你的 App → Settings → Secrets → 粘贴上方配置 → Save → Reboot app。</div>",
                unsafe_allow_html=True,
            )
...
def render_left_panel() -> None:
    ...
    if uploaded_file is not None:
        ...
        if st.button("分析款式资产", type="primary", use_container_width=True):
            ...
            try:
                original_mb = len(image_bytes) / 1024 / 1024
                step1.success(f"Step 1/5：读取上传图片成功，原始大小 {original_mb:.1f}MB。")

                api_image_bytes, mime_type, compress_note = compress_image_for_api(image_bytes, max_mb=2.0)
                api_mb = len(api_image_bytes) / 1024 / 1024
                step2.success(f"Step 2/5：图片预处理成功。{compress_note}")

                step3.info("Step 3/5：准备调用 Vision Model。当前会把图片作为 image_url 兼容格式发送。")

                with st.spinner("Step 4/5：请求已发送，等待图片理解模型返回，最多等待 60 秒..."):
                    step4.info(f"Step 4/5：正在调用图片理解模型。发送图片大小 {api_mb:.1f}MB，MIME={mime_type}。")
                ...
                if result and result.strip():
                    st.session_state.clothing_profile = result
                    step5.success("Step 5/5：模型返回成功，款式资产卡已生成。")
                    st.success("款式资产卡已生成")
                else:
                    step5.error("Step 5/5：模型返回为空。请检查 Vision Endpoint 是否支持图片理解。")

            except Exception as exc:
                step5.error(f"Step 5/5：分析失败：{exc}")
                st.markdown(
                    "<div class='warning-soft'>排查顺序：1）先点顶部“测试文本连接”；2）再点“测试图片理解”；3）如果文本成功、图片失败，切换图片传输格式 data_url/raw_base64；4）仍失败则需要单独配置支持图片理解的 ARK_VISION_MODEL_ID。</div>",
                    unsafe_allow_html=True,
                )
