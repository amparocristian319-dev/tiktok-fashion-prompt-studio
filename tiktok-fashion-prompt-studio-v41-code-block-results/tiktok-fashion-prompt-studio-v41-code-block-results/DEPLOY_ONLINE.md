...
- 修复 V37 中 `BASE_DIR = Path(__file__).resolve().parent` 报错的问题。
- 原因：V37 把配置读取逻辑合并进 app.py，但漏加了 `from pathlib import Path` 和 `import json`。
- V38 已补齐导入，避免 NameError。



## V39 更新：自定义数量 + 刷新恢复

- 输出数量改成自定义数字输入框，范围 1-50。
- 刷新页面后自动恢复：
  - 款式资产卡
  - 图片 URL
  - 七牛上传诊断
  ...
  - 本批场景规划
  - 已生成结果
- 每完成一条提示词就会自动保存一次，避免跑到一半刷新后全部丢失。
- 注意：如果正在 API 请求中刷新页面，当前请求可能中断；已完成并保存的内容会恢复。
- Streamlit Cloud 重新部署、重启、休眠后，本地缓存可能失效；长期生产建议接 Supabase/PostgreSQL。



## V40 更新：目标数量稳定生成 + 原创内容约束

- 修复“设置 10 条但只生成 2 条”的问题：
  - 单条失败不再中断整批。
  - 每条支持主生成 + 快速兜底。
  - 直到完成目标数量，或达到最大尝试次数。
- 参考资料不写进代码、不作为内容库：
  - 只保留通用输出结构。
  - 所有场景、道具、台词、人物关系都要求重新生成。
- 男友视角不禁止，可以继续使用，但不会固定复用。
- 增加视觉真实性硬约束：
  - 如果资产卡没明确确认口袋，禁止写 pocket / 口袋 / 插兜 / it has pockets。
  - 如果资产卡没明确确认鞋子/包袋，不指定具体鞋款或包款。
- 继续保留自定义输出数量、刷新恢复、结果卡片折叠查看。



## V41 更新：Gemini 风格代码块结果

- 生成结果不再用大文本框、下拉快速查看、折叠摘要混合展示。
- 改为：一个提示词一个代码块。
- 默认只展开 Video 01，其他 Video 折叠。
- 每个代码块下方保留单条 TXT 下载按钮。
- 保留下载全部 TXT / CSV。
