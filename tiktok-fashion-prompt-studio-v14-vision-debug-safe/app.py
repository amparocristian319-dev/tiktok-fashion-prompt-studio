...
def render_api_center() -> None:
    ...
    with st.expander("API 配置中心 · Doubao Seed 2.1 Pro", expanded=not is_configured):
        ...
        with test_col2:
            st.caption("图片理解测试请用左侧上传小图后点“分析款式资产”。V13 已加 60 秒超时和自动压缩。")
        ...
        if st.button("分析款式资产", type="primary", use_container_width=True):
            with st.spinner("正在调用图片理解模型分析服装款式，最多等待 60 秒..."):
                try:
                    ...
                    st.error(f"分析失败：{exc}")
                    st.markdown(
                        "<div class='warning-soft'>如果这里提示 image_url / unsupported content type，说明当前 Endpoint 不支持图片理解，需要把 ARK_VISION_MODEL_ID 换成支持图片理解的模型。</div>",
                        unsafe_allow_html=True,
                    )
